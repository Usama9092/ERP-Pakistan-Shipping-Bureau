...
def _create_project():
    ...
    with st.form("gm_create_project_v14"):
        ...
        dms=pq.users('dm')
        dm_id=st.selectbox("Department Manager",[u['id'] for u in dms],format_func=lambda x:next(u['full_name'] for u in dms if u['id']==x)) if dms else None

        # Reusable stakeholder registry — GM selects existing records created from
        # the Projects → + New Stakeholder page. No free-text stakeholder names.
        try:
            stakeholder_registry = pq.stakeholder_registry_v414()
        except Exception:
            stakeholder_registry = []
        by_type = {}
        for row in stakeholder_registry:
            by_type.setdefault(row.get('stakeholder_type'), []).append(row)

        st.markdown("**Project Stakeholders**")
        st.caption("Select registered stakeholder organizations. To add a new organization, use '+ New Stakeholder' beside Create Project in the Projects tab.")

        def _stakeholder_options(kind):
            return by_type.get(kind, [])

        owner_rows = _stakeholder_options('owner')
        designer_rows = _stakeholder_options('designer')
        shipm_rows = _stakeholder_options('ship_management')
        shipyard_rows = _stakeholder_options('shipyard')

        def _fmt_stakeholder(row):
            return f"{row.get('company_name','—')} · {row.get('contact_name') or 'No contact'}"

        owner_choices = ['none'] + [r['id'] for r in owner_rows]
        designer_choices = ['none'] + [r['id'] for r in designer_rows]
        shipm_choices = ['none'] + [r['id'] for r in shipm_rows]
        shipyard_choices = ['none'] + [r['id'] for r in shipyard_rows]

        sc1, sc2 = st.columns(2)
        with sc1:
            owner_registry_id = st.selectbox("Owner", owner_choices, format_func=lambda x: 'Not assigned' if x == 'none' else _fmt_stakeholder(next(r for r in owner_rows if r['id']==x)), key='gm_project_owner_registry')
            designer_registry_id = st.selectbox("Designer", designer_choices, format_func=lambda x: 'Not assigned' if x == 'none' else _fmt_stakeholder(next(r for r in designer_rows if r['id']==x)), key='gm_project_designer_registry')
        with sc2:
            shipm_registry_id = st.selectbox("Ship Management", shipm_choices, format_func=lambda x: 'Not assigned' if x == 'none' else _fmt_stakeholder(next(r for r in shipm_rows if r['id']==x)), key='gm_project_shipm_registry')
            shipyard_registry_id = st.selectbox("Shipyard", shipyard_choices, format_func=lambda x: 'Not assigned' if x == 'none' else _fmt_stakeholder(next(r for r in shipyard_rows if r['id']==x)), key='gm_project_shipyard_registry')

        # Optional link to an existing authenticated stakeholder account for direct project access.
        designers=pq.users('designer')
        designer_user=st.selectbox("Linked Designer account (optional)",['none']+[u['id'] for u in designers],format_func=lambda x:'Not linked' if x=='none' else next(u['full_name'] for u in designers if u['id']==x)) if designers else 'none'
        shipm_users=pq.users('ship_management')
        shipm_user=st.selectbox("Linked Ship Management account (optional)",['none']+[u['id'] for u in shipm_users],format_func=lambda x:'Not linked' if x=='none' else next(u['full_name'] for u in shipm_users if u['id']==x)) if shipm_users else 'none'
        shipyard_users=pq.users('shipyard')
        shipyard_user=st.selectbox("Linked Shipyard account (optional)",['none']+[u['id'] for u in shipyard_users],format_func=lambda x:'Not linked' if x=='none' else next(u['full_name'] for u in shipyard_users if u['id']==x)) if shipyard_users else 'none'
        owner_users=pq.users('owner')
        owner_user=st.selectbox("Linked Owner account (optional)",['none']+[u['id'] for u in owner_users],format_func=lambda x:'Not linked' if x=='none' else next(u['full_name'] for u in owner_users if u['id']==x)) if owner_users else 'none'
    ...
    if submit:
        if not name.strip() or not vessel_type.strip() or not flag.strip() or not phases:
            ...
            return
        team=[]
        if dm_id: team.append({'user_id':dm_id,'role':'dm','discipline':''})

        # Resolve selected stakeholder registry entries into the immutable project payload.
        registry_by_id = {r['id']: r for r in stakeholder_registry}
        stakeholders=[]
        for registry_id, typ, linked in [
            (owner_registry_id, 'owner', owner_user),
            (designer_registry_id, 'designer', designer_user),
            (shipm_registry_id, 'ship_management', shipm_user),
            (shipyard_registry_id, 'shipyard', shipyard_user),
        ]:
            if registry_id and registry_id != 'none':
                ...
                if reg:
                    stakeholders.append({
                        ...
                        'user_id': None if linked == 'none' else linked,
                    })

        # Phase-specific stakeholder requirements.
        missing=[]
        if 'plan_appraisal' in phases and designer_registry_id == 'none':
            missing.append('Designer')
        if 'nsc_survey' in phases and shipyard_registry_id == 'none':
            missing.append('Shipyard')
        if 'in_service' in phases and owner_registry_id == 'none':
            missing.append('Owner')
        if 'in_service' in phases and shipm_registry_id == 'none':
            missing.append('Ship Management')
        if missing:
            st.error('Select required stakeholder records before creating the project: ' + ', '.join(missing))
            return

        selected_owner = registry_by_id.get(owner_registry_id) if owner_registry_id != 'none' else None
        owner_company = selected_owner.get('company_name','') if selected_owner else ''
