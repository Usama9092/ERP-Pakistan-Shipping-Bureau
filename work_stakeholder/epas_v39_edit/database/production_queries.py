...
def stakeholders(project_id: str) -> list[dict]:
    return _cached(
        make_key("stakeholders", project_id),
        lambda: _db().table("stakeholders").select(
            "id,project_id,registry_id,company_name,contact_name,contact_email,contact_phone,contact_designation,registration_no,stakeholder_type,status"
        ).eq("project_id", project_id).order("stakeholder_type").execute().data,
        20.0,
    )


def stakeholder_registry(stakeholder_type: str | None = None, active_only: bool = True) -> list[dict]:
    key = make_key("stakeholder_registry", stakeholder_type or "all", active_only)
    def _load():
        q = _db().rpc("epas_list_stakeholder_registry", {"p_stakeholder_type": stakeholder_type})
        rows = q.execute().data or []
        if not active_only:
            return rows
        return [r for r in rows if str(r.get("status", "active")).lower() == "active"]
    return _cached(key, _load, 15.0)


def create_stakeholder(payload: dict) -> dict:
    result = _rpc("epas_create_stakeholder", {
        "p_stakeholder_type": payload.get("stakeholder_type"),
        "p_company_name": payload.get("company_name"),
        "p_registration_no": payload.get("registration_no"),
        "p_country": payload.get("country"),
        "p_address": payload.get("address"),
        "p_city": payload.get("city"),
        "p_postal_code": payload.get("postal_code"),
        "p_website": payload.get("website"),
        "p_contact_name": payload.get("contact_name"),
        "p_contact_designation": payload.get("contact_designation"),
        "p_contact_email": payload.get("contact_email"),
        "p_contact_phone": payload.get("contact_phone"),
        "p_contact_mobile": payload.get("contact_mobile"),
        "p_notes": payload.get("notes"),
    }, ["stakeholder_registry", "projects", "stakeholders"])
    return result or {}
...
# ---------------------------------------------------------------------------
# v4.1.4 reusable stakeholder registry
# ---------------------------------------------------------------------------
def stakeholder_registry_v414(stakeholder_type: str | None = None):
    return stakeholder_registry(stakeholder_type=stakeholder_type, active_only=True)

def create_stakeholder_v414(payload: dict):
    return create_stakeholder(payload)
