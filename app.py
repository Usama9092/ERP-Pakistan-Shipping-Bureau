...
from maritime_module import build_theme_css
...
LOGIN_GUARD = LoginAttemptGuard(max_attempts=LOGIN_LOCKOUT_ATTEMPTS)
...
def ensure_strong_password(password: str) -> bool:
    return PASSWORD_SECURITY.is_password_strong(password)
...
def ensure_default_ceo_user() -> None:
    ...
    try:
        ...
        if ceo.empty:
            default_password = os.getenv("DEFAULT_CEO_PASSWORD", "CEO@Psb2026!")
            db_insert("users", {
                ...
                "login_id": "ceo", "password_hash": phash(default_password), "temp_password": default_password,
...
def apply_style() -> None:
    ...
    css = (
        "<style>"
        "\n:root{--psb-navy:#071225;--psb-blue:#0b3b76;--psb-sky:#124f9e;--psb-card:var(--psb-surface-alt);--psb-line:var(--psb-line);--psb-text:var(--psb-text);--psb-muted:var(--psb-muted)}"
        "\n"
        + theme_css
        + "\n.stApp{background:linear-gradient(135deg,var(--psb-shell-bg) 0%,var(--psb-surface) 52%,var(--psb-surface-alt) 100%);color:var(--psb-text)}"
        + "\n.block-container{padding-top:1rem;padding-bottom:2.5rem;max-width:1480px}"
        + "\n#MainMenu, footer, header[data-testid=\"stHeader\"]{visibility:hidden}"
        + "\nsection[data-testid=\"stSidebar\"]{background:var(--psb-surface-alt);border-right:1px solid var(--psb-line)}"
        + "\nsection[data-testid=\"stSidebar\"] *{color:var(--psb-text)}"
        ...
        + "\n.psb-hero{background:var(--psb-hero-bg);color:var(--psb-hero-text);padding:1.55rem 1.85rem;border-radius:30px;margin-bottom:1.3rem;box-shadow:0 26px 75px rgba(15,23,42,.12);display:flex;gap:20px;align-items:center;border:1px solid var(--psb-line)}"
        + "\n.psb-hero img{width:96px;height:96px;border-radius:50%;object-fit:contain;background:var(--psb-surface-alt);padding:6px;box-shadow:0 14px 34px rgba(0,0,0,.12)}"
        + "\n.psb-hero h1{margin:0;font-size:2.18rem;letter-spacing:-.035em;font-weight:900}"
        + "\n.psb-hero p{color:var(--psb-muted);margin:.42rem 0 .25rem;font-size:1.03rem}"
        + "\n.pill{display:inline-flex;padding:6px 12px;border-radius:999px;background:var(--psb-hero-pill-bg);color:var(--psb-hero-pill-text);font-size:12px;font-weight:800;margin:4px 5px 4px 0;border:1px solid var(--psb-line);white-space:nowrap}"
        + "\n.psb-hero .pill{background:var(--psb-hero-pill-bg);color:var(--psb-hero-pill-text);border:1px solid var(--psb-line)}"
        + "\n.step{border-left:5px solid var(--psb-blue);background:var(--psb-surface-alt);color:var(--psb-text);border:1px solid var(--psb-line);border-radius:18px;padding:.9rem 1rem;margin:.48rem 0;box-shadow:0 12px 32px rgba(15,23,42,.07)}"
        + "\n.psb-card{background:var(--psb-surface-alt);color:var(--psb-text);border:1px solid var(--psb-line);border-radius:22px;padding:1rem 1.1rem;margin:.65rem 0;box-shadow:0 12px 32px rgba(15,23,42,.07)}"
        ...
        + "\n.login-frame{width:min(1180px,96vw);display:grid;grid-template-columns:1.08fr .92fr;gap:0;background:var(--psb-surface-alt);border:1px solid var(--psb-line);border-radius:36px;overflow:hidden;box-shadow:0 38px 110px rgba(7,18,37,.22);backdrop-filter:blur(14px)}"
        + "\n.login-brand{position:relative;padding:3rem 2.8rem;color:var(--psb-brand-text);background:var(--psb-brand-bg);min-height:650px;display:flex;flex-direction:column;justify-content:space-between;border-right:1px solid var(--psb-line)}"
        + "\n.login-brand:before{content:\"\";position:absolute;inset:0;background:radial-gradient(circle at 20% 20%,rgba(15,35,68,.08),transparent 18%),radial-gradient(circle at 85% 15%,rgba(255,255,255,.18),transparent 22%);opacity:.75;pointer-events:none}"
        ...
        + "\n.login-logo-row img{width:86px;height:86px;border-radius:22px;background:var(--psb-surface-alt);padding:8px;object-fit:contain;box-shadow:0 18px 45px rgba(0,0,0,.12)}"
        + "\n.login-kicker{font-size:.78rem;font-weight:900;text-transform:uppercase;letter-spacing:.18em;color:var(--psb-accent);margin-bottom:.4rem}"
        + "\n.login-brand h1{margin:0;font-size:2.65rem;line-height:1.04;letter-spacing:-.055em;color:var(--psb-brand-text);font-weight:950}"
        + "\n.login-brand p{font-size:1.03rem;line-height:1.65;color:var(--psb-muted);max-width:610px;margin:1.05rem 0}"
        + "\n.login-badges{display:flex;gap:9px;flex-wrap:wrap;margin:1.25rem 0 0}"
        + "\n.login-badge{display:inline-flex;align-items:center;border:1px solid var(--psb-line);background:var(--psb-badge-bg);border-radius:999px;padding:7px 11px;color:var(--psb-badge-text);font-size:.78rem;font-weight:850}"
        + "\n.login-feature-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:1.5rem}"
        + "\n.login-feature{border:1px solid var(--psb-line);background:var(--psb-feature-bg);border-radius:18px;padding:13px 14px;color:var(--psb-feature-text)}"
        + "\n.login-feature b{display:block;color:var(--psb-brand-text);font-size:.92rem;margin-bottom:4px}.login-feature span{font-size:.78rem;color:var(--psb-feature-text)}"
        + "\n.brand-footer{border-top:1px solid var(--psb-line);padding-top:1rem;color:var(--psb-muted);font-size:.82rem;display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap}"
        + "\n.login-panel{padding:3rem 2.6rem;background:var(--psb-surface);display:flex;flex-direction:column;justify-content:center}"
        + "\n.login-card{background:var(--psb-card);border:1px solid var(--psb-line);border-radius:30px;padding:2rem;box-shadow:0 18px 40px var(--psb-card-shadow)}"
        + "\n.login-card h2{font-size:1.75rem;margin:0 0 .35rem;color:var(--psb-text);font-weight:950}.login-card .muted{color:var(--psb-muted);margin:0 0 1.25rem;line-height:1.55}"
        + "\n.login-mini{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:1.1rem 0 0}.login-mini div{background:var(--psb-surface);border:1px solid var(--psb-line);border-radius:16px;padding:10px;text-align:center}.login-mini b{display:block;color:var(--psb-accent);font-size:1rem}.login-mini span{font-size:.70rem;color:var(--psb-muted);font-weight:800;text-transform:uppercase;letter-spacing:.05em}"
        + "\n.login-help{margin-top:1rem;padding:12px 14px;border-radius:18px;background:var(--psb-surface);border:1px solid var(--psb-line);color:var(--psb-text);font-size:.86rem;line-height:1.5}"
        + "\n.login-card div[data-testid=\"stForm\"]{border:0;padding:0}.login-card label{font-weight:850;color:var(--psb-text)!important}.login-card input{border-radius:14px!important}"
        + "\n.login-card .stButton>button{width:100%;height:3rem;border-radius:16px;background:linear-gradient(135deg,var(--psb-accent),var(--psb-accent-strong));border:0;color:white;font-weight:950;letter-spacing:.02em;box-shadow:0 14px 32px var(--psb-button-shadow)}"
        + "\n.login-card .stButton>button:hover{background:linear-gradient(135deg,rgba(15,79,141,.95),var(--psb-accent-strong));transform:translateY(-1px)}"
        + "\n.login-demo{margin-top:1rem}.login-demo div[data-testid=\"stExpander\"]{box-shadow:none;border-radius:18px;background:var(--psb-surface-alt)}"
        ...
        + "\n.stButton>button,.stDownloadButton>button{border-radius:13px;border:1px solid var(--psb-accent);background:var(--psb-accent);color:white;font-weight:800;box-shadow:0 8px 18px var(--psb-button-shadow)}"
        + "\n.stButton>button:hover,.stDownloadButton>button:hover{background:var(--psb-accent-strong);color:white;border-color:var(--psb-accent-strong)}"
        ...
        + "\ndiv[data-testid=\"stExpander\"]{border-radius:18px;border:1px solid var(--psb-line);background:var(--psb-surface-alt);box-shadow:0 8px 22px rgba(15,23,42,.04)}"
        + "\n.stTabs [data-baseweb=\"tab-list\"]{gap:8px}"
        + "\n.stTabs [data-baseweb=\"tab\"]{border-radius:999px;background:var(--psb-surface-elevated);padding:.45rem 1rem;font-weight:800}"
...
def login_page() -> None:
    ...
    if submit:
        ...
        if LOGIN_GUARD.is_locked(login_key, st.session_state):
            remaining = LOGIN_GUARD.remaining_seconds(login_key, st.session_state)
            st.error(f"Account temporarily locked. Please try again in {remaining} seconds.")
            st.stop()
        match = db_where(
            "users",
            "(lower(login_id) = :login_key or lower(email) = :login_key) and status = 'Active'",
            (("login_key", login_key),),
        )
        if match.empty:
            LOGIN_GUARD.record_failure(login_key, st.session_state)
            st.error("Invalid login ID/email or password.")
        else:
            user = match.iloc[0].to_dict()
            stored_hash = clean(user.get("password_hash", ""))
            if not verify_password(password.strip(), stored_hash):
                LOGIN_GUARD.record_failure(login_key, st.session_state)
                st.error("Invalid login ID/email or password.")
            else:
                LOGIN_GUARD.record_success(login_key, st.session_state)
                st.session_state["logged_in"] = True
                st.session_state["user"] = user
                SESSION_SECURITY.issue_session_token(user.get("user_id", ""), user.get("role", ""), st.session_state)
...
def require_login() -> dict:
    ...
    if not st.session_state["logged_in"]:
        login_page()
        st.stop()
    if not SESSION_SECURITY.validate_session_token(st.session_state):
        st.session_state["logged_in"] = False
        st.session_state["user"] = {}
        SESSION_SECURITY.clear_session(st.session_state)
